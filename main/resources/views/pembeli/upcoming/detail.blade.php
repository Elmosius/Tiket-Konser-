@extends('layouts.pembeli-penjual-guess.master2')
    ...
    <section class="container my-5 px-32">
        ...
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('upcoming.detail', ['event' => $event->id]) }}">Pertunjukan
                        Konser</a></li>
        ...
        <div class="row">
            ...
            <div class="col-lg-4">
                <div class="rounded shadow-sm p-4 bg-white mb-4">
                    ...
                    <p class="mb-2"><i class="fa fa-clock"></i> <a href="#" class="text-primary">Lihat jadwal</a>
                    </p>
                ...
                <div class="rounded shadow-sm p-4 bg-white">
                    ...
                    <button type="button" id="beli-tiket-btn" class="btn btn-primary w-100 py-2 mt-3"
                        onclick="submitForm()" disabled>Beli Tiket</button>
        ...
        <div class="mt-5">
            <div class="d-flex justify-content-center align-items-center mb-3">
                <button class="btn btn-outline-primary px-4 py-2 mx-2 active" id="deskripsi-tab"
                    onclick="showTab('deskripsi')">Deskripsi</button>
                <button class="btn btn-outline-primary px-4 py-2 mx-2" id="tiket-tab"
                    onclick="showTab('tiket')">Tiket</button>
            </div>
                ...
                <div id="tiket" class="tab-pane fade">
                    <form action="{{ route('simpan-pembelian') }}" method="post" id="ticketForm">
                        ...
                        @foreach ($tikets as $index => $tiket)
                            ...
                            <!-- Tiket Card -->
                            <div class="d-flex flex-column shadow rounded-lg p-4 mb-4"
                                style="background: #fff; border: 1px solid #ddd;">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        <h5 class="fw-bold mb-1">{{ $tiket->nama_tiket }}</h5>
                                        <p class="text-muted mb-0"><i class="fa fa-calendar-alt"></i>
                                            {{ \Carbon\Carbon::parse($tiket->tanggal_mulai)->locale('id')->format('d F Y - h:m') }}
                                        </p>
                                    </div>
                                    <div>
                                        <h4 class="fw-bold text-primary mb-0">Rp.
                                            {{ number_format($tiket->harga, 0, ',', '.') }}</h4>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center"
                                    id="ticket-{{ $tiket->id }}">
                                    <div>
                                        <p class="text-muted mb-0">Kode Tiket: <span
                                                class="fw-bold">{{ $tiket->id }}</span></p>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <button type="button" class="btn btn-outline-primary btn-sm mx-2"
                                            onclick="updateTicketCount('{{ $tiket->id }}', -1)">
                                            <i class="fa fa-minus"></i>
                                        </button>
                                        <input type="number" class="no-spinner" name="tickets[{{ $tiket->id }}]"
                                            id="ticket-count-{{ $tiket->id }}" max="{{ $event->pembelian_maksimum }}"
                                            value="{{ old('tickets.' . $tiket->id, 0) }}">
                                        <button type="button" class="btn btn-outline-primary btn-sm mx-2"
                                            onclick="updateTicketCount('{{ $tiket->id }}', 1)">
